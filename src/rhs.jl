"""
    rhs_signaling!(du, u, p, t)

In-place ODE right-hand side for the Gong et al. (2020) beta-adrenergic signaling model.
Computes the derivatives of all 57 state variables. Suitable for direct use with
OrdinaryDiffEq.jl: `ODEProblem(rhs_signaling!, u0, tspan, p)`.

# Arguments
- `du`: output derivative vector (57 elements), modified in place
- `u`: current state vector (57 elements)
- `p`: parameter vector (167 elements) from [`compute_parameters`](@ref)
- `t`: time (unused; the model is autonomous)
"""
